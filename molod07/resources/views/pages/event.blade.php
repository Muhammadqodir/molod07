    ...
    <section class="bg-accentbg mt-[-80px] pt-[80px]">
        <div class="max-w-6xl mx-auto py-6 space-y-8" x-data="{ tab: 'info', roleIdx: 0 }">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-6 p-4 md:p-6">
                    ...
                    <div class="md:col-span-8 flex flex-col gap-3">
                        <div class="flex items-center gap-2 flex-wrap text-sm">
                            @if ($event->type)
                                <span class="inline-flex items-center gap-2 bg-gray-100 text-gray-700 px-3 py-1 rounded-xl">
                            ...
                            @if ($sumPoints)
                                <span
                                    class="inline-flex items-center gap-2 bg-green-100 text-green-700 px-3 py-1 rounded-xl">
                                    <x-lucide-badge-russian-ruble class="w-4 h-4" />
                                    {{ $sumPoints }} баллов
                                </span>
                            @endif
                        </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm p-4 md:p-6">
                ...
                <h2 class="text-lg font-semibold">Информация о мероприятии</h2>
                    ...
                    <section class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <div
                                        class="w-8 h-8 rounded-xl bg-gray-100 flex items-center justify-center text-gray-600">
                                        <x-lucide-user class="w-4 h-4" />
                                    </div>
                                    <div class="text-sm text-primary">
                                        {{-- сюда можно вывести партнёра/организацию при наличии --}}
                                        {{ $event->partnersProfile->org_name ?? 'Организация' }}
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <img src="{{ $event->supervisor_pic_url ?? '/images/avatar-placeholder.png' }}"
                                        class="w-8 h-8 rounded-xl object-cover" alt="">
                                    <div class="text-sm">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="text-sm text-gray-700 flex items-start gap-2">
                                    ...
                                    <div>
                                        {{ $event->settlement ?? '—' }}, {{ $event->address ?? '—' }} <br>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center gap-3 text-gray-600">
                                @if ($event->vk)
                                    <a href="{{ $event->vk }}" target="_blank" class="p-2 hover:bg-gray-100 rounded-xl">
                                        <x-lucide-vibrate class="hidden" /> {{-- заглушка иконки; вставь свою VK --}}
                                ...
                                @endif
                                @if ($event->web)
                                    <a href="{{ $event->web }}" target="_blank" class="p-2 hover:bg-gray-100 rounded-xl">
                ...
                {{-- Tab: Media --}}
                <div x-show="tab==='media'" class="p-4 md:p-6 space-y-8">
                    {{-- Docs --}}
                    @if ($docs)
                        <section class="space-y-3">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                @foreach ($docs as $d)
                                    <a href="{{ is_array($d) ? $d['url'] ?? '#' : $d }}" target="_blank"
                                        class="flex items-center justify-between bg-[#f6f8fe] px-4 py-3 rounded-xl hover:bg-[#eef2ff]">
                                        <div class="truncate text-gray-700">
                                            {{ is_array($d) ? $d['name'] ?? basename($d['url'] ?? 'документ') : basename($d) }}
                                        </div>
                                        <x-lucide-download class="w-5 h-5 text-gray-500" />
                                    </a>
                                @endforeach
                            </div>
                        </section>
                        ...
                        <section class="space-y-3">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                @foreach ($images as $img)
                                    <a href="{{ is_array($img) ? $img['url'] ?? '#' : $img }}" target="_blank"
                    ...
                    @if ($videos)
                        <section class="space-y-3">
                            <h3 class="text-lg font-semibold">Видео</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
