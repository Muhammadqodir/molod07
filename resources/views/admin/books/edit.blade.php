...
@section('content')
    <div class="flex items-center justify-between mb-0">
        <h1 class="text-3xl">Редактировать книгу</h1>
    </div>
    ...
    <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <section class="space-y-3">
            ...
            @if($book->cover)
                <div class="mb-3">
                    ...
                    <img src="{{ asset($book->cover) }}" alt="Current cover" class="w-32 h-40 object-cover rounded-lg border shadow-sm">
                </div>
            @endif
        ...
        <section class="space-y-3">
            ...
            <x-input label="Название книги" name="title" placeholder="Введите название книги" value="{{ old('title', $book->title) }}" required />

            <x-input label="Автор" name="author" placeholder="Введите имя автора" value="{{ old('author', $book->author) }}" required />

            <x-multiline-input label="Краткое описание / аннотация" name="description"
                placeholder="Введите краткое описание или аннотацию книги">{{ old('description', $book->description) }}</x-multiline-input>

            <x-input label="Ссылка на электронную версию" name="link" type="url" placeholder="https://example.com/book.pdf"
                value="{{ old('link', $book->link) }}" />
