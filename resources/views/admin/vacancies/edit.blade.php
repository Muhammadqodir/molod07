...
@section('content')
    ...
    <form action="{{ route(Auth::user()->role . '.vacancies.update', $vacancy->id) }}" method="POST" class="space-y-5">
        ...
        <section class="space-y-3">
            ...
            <div class="space-y-2">
                <x-multi-choice name="category" :options="$categories" :value="old('category') ? [old('category')] : ($vacancy->category ? [$vacancy->category] : [])" :multiple="false" title="Категория"
            ...
            <div class="space-y-2">
                <x-multi-choice name="type" :options="$types" :value="old('type') ? [old('type')] : ($vacancy->type ? [$vacancy->type] : [])" :multiple="false" title="Тип занятости"
                    hint="Выберите один вариант" />
                @error('type')
                    <p class="text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="space-y-2">
                <x-multi-choice name="experience" :options="$experience_levels" :value="old('experience') ? [old('experience')] : ($vacancy->experience ? [$vacancy->experience] : [])" :multiple="false"
