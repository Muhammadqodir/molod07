...
@section('title', 'Все отклики на вакансии')

@section('content')
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-3xl">Все отклики на вакансии</h1>
    ...
    <form method="GET" action="{{ route('admin.vacancies.responses') }}" class="mb-6">
        <div class="relative mt-4">
            <x-search-input name="q" placeholder="Поиск по вакансии, соискателю или работодателю" :value="old('q', request('q'))" />
        </div>
    </form>

    @if ($responses->isEmpty())
        <x-empty title="Пока нет откликов на вакансии." />
    @else
        {{-- Table --}}
                ...
                <tbody>
                    @foreach ($responses as $response)
                        ...
                        <tr class="border-t border-gray-100">
                            ...
                            <td class="px-4 py-4">
                                <div class="text-gray-800 font-medium">{{ $response->vacancy->org_name }}</div>
                                @if($response->vacancy->org_email)
                                    <div class="text-xs text-gray-500">{{ $response->vacancy->org_email }}</div>
                                @endif
                                @if($response->vacancy->org_phone)
                                    <div class="text-xs text-gray-500">{{ $response->vacancy->org_phone }}</div>
                                @endif
                            </td>
                            ...
                            <td class="px-2 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <a href="{{ route('admin.vacancies.preview', $response->vacancy->id) }}" target="_blank" title="Открыть вакансию">
                                        <x-nav-icon>
